...
async function main(){
	...
	let handleFuction = function () { 																						//! cuando una funcion callback es tan grande, no es mejor definirla como funcion normal?
		
		let pescador = document.querySelector(`.datosTarjeta`); //~ DOM

		//* Si ya esta abierto lo cierra																					
		if (this.classList.contains('Abre')) {																				//! uso de this por problemas en el eventlistener.
		...
		document.querySelectorAll(".capitulos").forEach((tarjeta) => tarjeta.addEventListener("click",async (event)=>{ //~DOM
			try {
				...
				let pescador = document.querySelector(`#${this.id} .datosTarjeta`); //~DOM
	...
	window.addEventListener('scroll', async function () { 																				//! funcion de flecha asyncrona?
